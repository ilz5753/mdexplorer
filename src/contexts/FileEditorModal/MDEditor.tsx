import { MDPreview, type IMDPreview, isAndroid } from "@ilz5753/rnmd";
...
import { ReTextInput } from "@ilz5753/rnutils";
...
export function MDEditorRender({
  ...
  let horInpHeight = useAnimatedStyle(() => ({
    // height: interpolate(dy.value, [hz.value, mh.value], [hz.value, mh.value], "clamp"),
    height: ReClamp(dy.value, hz.value, mh.value),
  }));
  let horMdHeight = useAnimatedStyle(() => ({
    // height: interpolate(dy.value, [hz.value, mh.value], [hz.value, mh.value], "clamp"),
    height: ReClamp(hp.value - dy.value, hz.value, mh.value),
  }));
  ...
  let PanYBg = useAnimatedStyle(() => ({
    ...
    transform: [
      {
        translateY: dy.value - hz.value,
        // translateY: ReClamp(dy.value-hz.value, hz.value, mh.value),
      },
    ],
  }));
      ...
      <Animated.View {...{ style: [styles.f1, BG] }}>
        ...
        <Animated.View {...{ style: [styles.f1] }}>
          ...
          {horizontal ? (
            ...
            <Animated.View {...{ style: [styles.f1, { borderWidth: 1 }] }}>
              {/* <Animated.View
                {...{
                  style: [styles.fw, { borderWidth: 1 }],
                }}>
                {ti}
              </Animated.View> */}
              <ReTextInput
              ...
              <Animated.View {...{ style: [styles.fw, horMdHeight] }}>
                {P}
              </Animated.View>
            </Animated.View>
          )}
